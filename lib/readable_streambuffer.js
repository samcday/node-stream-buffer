...
var ReadableStreamBuffer = module.exports = function(opts) {
	...
	var sendData = function() {
		...
		var chunk = encoding ? buffer.toString(encoding, 0, Math.min(chunkSize, size)) : buffer.slice(0, Math.min(chunkSize, size));
		that.emit("data", chunk);

		if((size + chunk.length) < buffer.length)
			buffer.copy(buffer, 0, chunk.length, size);
		size -= chunk.length;
	};
	...
	this.put = function(data, encoding) {
		if(Buffer.isBuffer(data)) {
			...
			data = data + "";
			increaseBufferIfNecessary(data.length);
			buffer.write(data, 0, encoding || "utf8");
			size += data.length;
		}
	};
